"""
Callback Tests — before_model_callback (Guardrail Agent: trigger_handler)

Tests the trigger_handler callback on guardrail_agent.

guardrail_agent is the primary handler of advisor escalation (boundary-hit path).
The trigger is set by the LLM via set_session_state when a regulated-advice
boundary is hit. This callback fires deterministically and ensures the warm
handoff always executes even if the LLM doesn't issue the tool call itself.
...
WHAT THIS TESTS:
    ...
    escalate_to_advisor trigger:
        ...
        - Returns farewell text + end_session(session_escalated=True, reason='advisor_transfer')
...
def _ctx(state):
    ...
    ctx.user_content = Content(
        role="user",
        parts=[Part(text="Which ETF should I invest all my money in?")],
    )
    return ctx
...
# =============================================================================
# escalate_to_advisor (primary flow for guardrail_agent)
# =============================================================================

class TestEscalateToAdvisor:
    """escalate_to_advisor is the primary escalation path for guardrail_agent."""
    ...
    def _escalate_ctx(self, **extra):
        ...
        state = {
            ...
            "_escalation_open_question": "Customer asked for specific ETF recommendation.",
            **extra,
        }
        return _ctx(state)
    ...
    def test_calls_handoff_summary_with_open_question(self):
        ...
        python_code.tools.generate_advisor_handoff_summary.assert_called_once_with(
            open_question="Customer asked for specific ETF recommendation."
        )
    ...
    def test_farewell_includes_advisor_mention(self):
        """Guardrail agent farewell references licensed advisor."""
        result = before_model_callback(self._escalate_ctx(), llm_request=MagicMock())
        text = next(p.text for p in result.content.parts if getattr(p, "text", None))
        assert "advisor" in text.lower()

    def test_farewell_references_summary(self):
        """Guardrail agent farewell reassures customer they won't need to repeat themselves."""
        result = before_model_callback(self._escalate_ctx(), llm_request=MagicMock())
        text = next(p.text for p in result.content.parts if getattr(p, "text", None))
        assert "repeat" in text.lower() or "summary" in text.lower()
...
class TestEndSessionClean:
    """end_session_clean: clean close from guardrail_agent."""
...
class TestUnknownTrigger:
    """Unknown trigger cleared, returns None."""

    def test_unknown_trigger_returns_none(self):
        ctx = _ctx({"_action_trigger": "mystery_trigger"})
        result = before_model_callback(ctx, llm_request=MagicMock())
        assert result is None
