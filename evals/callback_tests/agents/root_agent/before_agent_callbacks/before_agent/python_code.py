...
CRITICAL:
    ...
    The early-return guards at the top of each phase are required to prevent
    resetting auth_status and account_snapshot on every turn.
...
def before_agent_callback(callback_context: CallbackContext) -> Optional[Content]:
    ...
    # =========================================================================
    # PHASE 1: auth_init
    # Guard: if auth_status already set (any non-empty value), skip.
    # =========================================================================
    if not state.get("auth_status"):
